...
export Variable, value, grad, backward!, zero_grad!, parameters, matmul, accumulate_gradient!, mean, tanh
...
mutable struct Variable{T<:Real}
    ...
    function Variable(val::Union{S, AbstractArray{S}}; is_param::Bool=false) where {S<:Real}
        empty_children = Vector{Variable{S}}()
...
function grad!(v::Variable{T}, g) where T
    ...
    if v.gradient === nothing
        ...
        accumulate_gradient!(v, g_converted)
    end
end
...
function Base.getindex(v::Variable{T}, args...) where T
    val = getindex(value(v), args...)
    original_shape = size(value(v))
    indices = args
    children = Variable[v]
    local new_var
    function backward_fn()
        slice_grad = grad(new_var)
        if slice_grad !== nothing && !all(iszero, slice_grad)
            ...
            accumulate_gradient!(v, full_grad)
        end
    end
...
function Base.:*(a::Variable{T}, b_scalar::Real) where T
    ...
    function backward_fn_scalar_multiply()
        output_grad = grad(new_var)
        if output_grad !== nothing
            grad_a_unshaped = output_grad .* b_scalar
...
function Statistics.mean(v::Variable{T}; dims) where T
    val = mean(value(v); dims=dims); original_shape = size(value(v)); num_elements_pooled = prod(size(value(v), d) for d in dims); N = T(num_elements_pooled); children = Variable[v]; local new_var
    function backward_fn(); output_grad = grad(new_var); if output_grad !== nothing && !all(iszero, output_grad); input_grad = similar(value(v)); input_grad .= output_grad ./ N; SimpleAutoDiff.accumulate_gradient!(v, input_grad); end; end
    new_var = Variable(val, children, backward_fn); return new_var
end
